# Starship and Zsh Setup

This repository contains a Starship prompt config and Zsh plugin setup guide. Each account installs the prompt and plugins into its own home directory, so normal users and root accounts do not share binaries, plugins, or shell config files.

## Clone this repository

```bash
git clone https://github.com/HungNth/starship-zsh-config.git
cd starship-zsh-config
```
...
### 2. Install Starship for this account
...
Ensure the account-local binary directory is on `PATH` in `~/.zshrc`:
...
### 4. Install Zsh plugins for this account
...
"$HOME/.local/bin/starship" print-config >/dev/null && echo "Starship config OK"
zsh -n "$HOME/.zshrc"
exec zsh
```
...
- If `starship` is not found, confirm `export PATH="$HOME/.local/bin:$PATH"` appears before Starship initialization in `.zshrc`.
